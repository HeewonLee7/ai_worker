...
#
# NOTE: This is a copy of ffw_sh5_follower_ai.launch.py with ONLY the
# arm_l/arm_r AND hand_l/hand_r joint_trajectory remaps removed. The original
# file is left untouched. The arm/hand controllers keep their default
# joint_trajectory topics so the exo_sync_gate node (in ffw_sh5_ai_handfix.launch.py)
# can sit in between:
#   - arm: strips gripper_l_joint1 / gripper_r_joint1 (bundled by the gripper-based
#     lg2 leader) so the arm controllers stop rejecting the trajectory.
#   - hand: forwards Manus commands ONLY while the exo arm is active, so the hand
#     starts/stops together with the exo trigger instead of moving the instant
#     Manus connects.
# Init-position moves are unaffected: joint_trajectory_executor uses the
# follow_joint_trajectory ACTION interface, not the joint_trajectory topic.
...
def generate_launch_description():
    ...
    robot_controller_spawner = Node(
        ...
        executable='spawner',
        arguments=[
            # arm_l/arm_r AND hand_l/hand_r are intentionally NOT remapped to the
            # leader/Manus topics here (unlike the original launch). They keep their
            # default joint_trajectory topics so the exo_sync_gate node can sit in
            # between (gripper strip for arms, exo-gated forwarding for hands).
            # head/lift remaps are unchanged.
            '--controller-ros-args',
