...
"""SH5 teleop (Gazebo) with lg2 (gripper) leader: exo drives arms/head/lift,
Manus drives hands -- synchronized so the hand only moves while the exo is active.

This is a copy of ffw_sh5_ai_gazebo.launch.py that adds an exo_sync_gate node so
the gripper-based lg2 leader can drive the SH5 (hand) arms AND so the Manus hand
stream is gated by the exo trigger:
...
original follower. No existing launch file is modified.

Install once (in container): sudo apt install ros-jazzy-rmw-cyclonedds-cpp

Run :
    ros2 launch ffw_bringup ffw_sh5_ai_gazebo.launch.py   # robot
...
def generate_launch_description():
    ...
    follower = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(
            bringup_launch_dir, 'ffw_sh5_follower_ai_gazebo_handfix.launch.py'))
    )
